...
// Triggered every 60s via external cron (Cloudflare Workers)
...
async function ingestDefiLlamaTvl(protocols: Protocol[]): Promise<Signal[]> {
  ...
  for (const protocol of protocols) {
    ...
    signals.push({
      ...
      metadata: { change_1h: data.change_1h, change_1d: data.change_1d },
      source: "defillama",
    });
    if (typeof data.change_1h === "number") {
      signals.push({
        ...
        metadata: { change_1h: data.change_1h },
        source: "defillama",
      });
    }
  }
...
async function runRuleEngine(
  ...
  for (const signal of signals) {
    ...
    for (const rule of applicableRules) {
      ...
      // Z-score override: extreme anomaly always escalates
  ...
  for (const [protocolId, protocolSignals] of signalsByProtocol) {
    for (const pattern of patterns) {
      ...
      for (const required of pattern.signals_required) {
        ...
        if (!signal) continue;
        let meets = required.threshold_pct !== undefined ? signal.value >= required.threshold_pct
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Ingest all sources in parallel — failures don't block others
